...
const SYN = {
  Name: ["Name","Component","Part","Item","Description","Panel Name","Label","Part Name"],
  Material: ["Material","Board","Material Name","Board Material","MaterialName"],
  // NEW: we’ll also auto-guess a dedicated MaterialTag if present
  MaterialTag: ["Material Tag","MaterialTag","Report Tag","Report Tags"],
  Length: ["Length","Long","DimY","Height","L","Len","Y","Panel Length","Cut Length","Long Side"],
  Width:  ["Width","DimX","X","W","Panel Width","Cut Width","Short Side"],
  Qty:    ["Qty","Quantity","QTY","Count","Pcs","Pieces","Q'ty","No","Number","Units"],
} as const;
...
export function autoMapHeaders(headers: string[], seed?: Partial<Mapping>): Partial<Mapping> {
  ...
  out.MaterialTag ??= find(SYN.MaterialTag);
