// src/ui/settings.ts
// Settings modal UI (touch-friendly). Uses state/settings exports.
...
export function openSettingsModal() {
  ...
  content.append(
    ...
    section("SVG Style", grid(
      row(color("Board Stroke", S.svgStyle?.boardStroke ?? "#1f2937"), "svgStyle.boardStroke"),
      ...
      row(color("Part Stroke", S.svgStyle?.partStroke ?? "#111827"), "svgStyle.partStroke"),
      ...
      row(number("Part Stroke Width", S.svgStyle?.partStrokeWidth ?? 2.5, 0.5, 0.5), "svgStyle.partStrokeWidth"),
      ...
      row(number("Touch Padding (px)", S.svgStyle?.touchTargetPadding ?? 4, 1, 1), "svgStyle.touchTargetPadding"),
...
function grid(...children: HTMLElement[]) {
  const g = el("div", { style: `display:grid;grid-template-columns: repeat(auto-fit,minmax(240px,1fr));gap:10px;padding:10px;` });
  g.append(...children);
  return g;
}
function row(inputEl: HTMLElement, path: string) {
  ...
  if (!(inputEl as HTMLInputElement).dataset?.label) cap.textContent = prettify(path);
...
function checkbox(label: string, checked: boolean) {
  const w = (document.createElement("div") as HTMLDivElement);
  const i = document.createElement("input");
  i.type = "checkbox";
  i.checked = !!checked;
  i.dataset.label = label;
...
function subhead(text: string) {
  return el("div", { text, style: "grid-column: 1 / -1; font-weight: 700; margin-top: 6px; opacity:.8;" });
}
function prettify(path: string) {
  return path.split(".").pop()!.replace(/([A-Z])/g, " $1").replace(/_/g, " ").replace(/\b\w/g, (m) => m.toUpperCase());
}
...
function setDeep(obj: any, path: string, val: any) {
  ...
  for (let i = 0; i < parts.length - 1; i++) { const k = parts[i]; if (!(k in cur)) cur[k] = {}; cur = cur[k]; }
