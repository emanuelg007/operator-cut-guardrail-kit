...
export function exportSvgToPng(svg: SVGSVGElement, filename = "sheet.png") {
  ...
  img.onload = () => {
    ...
    const w = vb && vb.width ? vb.width : svg.clientWidth || 1024;
    const h = vb && vb.height ? vb.height : svg.clientHeight || 768;
...
function escapeXml(s: string) { return s.replace(/[&<>"']/g, (ch) => ({ "&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&apos;" }[ch]!)); }
